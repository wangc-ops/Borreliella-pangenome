...
# Module: CDS_length
# Function: CDS length stratification (Core/Soft/Shell/Cloud)
# Statistics: Welch's ANOVA + Games-Howell post-hoc + CLD
# Input: ./data/gene_presence_absence_roary.csv, ./data/pan_genome_reference.fa
# Output: ./output/CDS_length_stratification.pdf
...
read_fasta_length <- function(fasta_file) {
  ...
  if (!is.null(cur_name)) seqs[[cur_name]] <- paste(cur_seq, collapse = "")
  data.table(gene_id = names(seqs), cds_length = nchar(seqs))
}
...
len_dt <- read_fasta_length(PATH_FASTA)
len_dt[, gene_prefix := sub("_.*$", "", gene_id)]
roary_len <- merge(roary, len_dt, by.x = "Gene", by.y = "gene_id", all.x = TRUE)
if (roary_len[!is.na(cds_length), .N] / nrow(roary) < 0.5) {
  roary_len <- merge(roary, len_dt, by.x = "Gene", by.y = "gene_prefix", all.x = TRUE)
}
plot_dt <- roary_len[!is.na(cds_length) & !is.na(strat)]

welch_res <- oneway.test(cds_length ~ strat, data = plot_dt, var.equal = FALSE)
gh <- games_howell_test(plot_dt$cds_length, plot_dt$strat)
cld_dt <- make_cld(gh$comparisons)

letter_dt <- merge(gh$stats, cld_dt, by.x = "group", by.y = "group")
letter_dt[, y_pos := max(plot_dt$cds_length, na.rm = TRUE) * 1.05]

p_cds <- ggplot(plot_dt, aes(x = strat, y = cds_length, fill = strat)) +
  ...
  labs(x = "Pan-genome category", y = "CDS length (bp)") +
  annotate("text", x = 2.5, y = max(plot_dt$cds_length, na.rm = TRUE) * 1.12,
           label = "Welch's ANOVA p < 0.001", size = 3.5) +
...
if (SAVE_OUTPUT) {
  ...
  ggsave(file.path(PATH_OUT, "CDS_length_stratification.pdf"), p_cds, width = 5.5, height = 4.8, dpi = 300)
}