# ============================================================
# pan_genome_diluted_analysis.R
# Diluted pan-genome Heaps curve and novel gene discovery rate
# for four Borreliella species (n=34 bootstrap)
# ============================================================
library(tidyverse)
library(readxl)
library(ggplot2)
...
# Heaps curve (diluted)
# ============================================================
calc_heaps_rarefied <- function(species_name, n_rare = N_RARE, n_perm = N_PERM) {
  gids_all <- valid_genomes %>% filter(species == species_name) %>% pull(Genome_ID)
  n_avail  <- length(gids_all)
  if (n_avail < n_rare) return(NULL)
  
  cum_matrix <- matrix(0, nrow = n_perm, ncol = n_rare)
  for (p in 1:n_perm) {
    sub_gids <- sample(gids_all, n_rare, replace = FALSE)
    pool <- character(0)
    for (i in 1:n_rare) {
      present <- rownames(pav_mat)[pav_mat[, sub_gids[i]] == 1]
      pool <- union(pool, present)
      cum_matrix[p, i] <- length(pool)
    }
  }
  tibble(species = species_name, n_genomes = 1:n_rare,
         mean_cum = colMeans(cum_matrix), sd_cum = apply(cum_matrix, 2, sd))
}

heaps_list <- map(sp_levels, calc_heaps_rarefied)
...
cat("=== Diluted power-law fit (n=34) ===\n")
print(heaps_fits)

legend_labels_a <- map_chr(sp_levels, function(sp) {
  n_orig <- n_per_species[sp]
  g <- heaps_fits$gamma[heaps_fits$species == sp]
  sp_short <- gsub("Borreliella ", "B. ", sp)
  if (n_orig == N_RARE) {
    sprintf("italic(\'%s\') ~ \' (n=%d, \u03B3=%.3f)\'", sp_short, n_orig, g)
  } else {
    sprintf("italic(\'%s\') ~ \' (n=%d/%d, \u03B3=%.3f)\'", sp_short, N_RARE, n_orig, g)
  }
})
...
heaps_plot <- ggplot(heaps_df, aes(x = n_genomes, y = mean_cum, color = species, fill = species)) +
  ...
  coord_cartesian(xlim = c(0, 34), ylim = c(600, 1600)) +
  scale_x_continuous(breaks = seq(0, 34, 5), expand = expansion(mult = c(0.02, 0.05))) +
...
print(heaps_plot)
ggsave(file.path(out_dir, "heaps_curve_diluted.png"), heaps_plot, 
       width = 7, height = 6, dpi = 300, bg = "white")

# ============================================================
# Novel gene discovery rate (diluted, Algorithm A + LOWESS)
# ============================================================
calc_novel_rarefied <- function(species_name, n_rare = N_RARE, n_perm = N_PERM) {
  gids_all <- valid_genomes %>% filter(species == species_name) %>% pull(Genome_ID)
  n_avail  <- length(gids_all)
  if (n_avail < n_rare) return(NULL)
  
  sp_size <- mean_genome_size %>% filter(species == species_name) %>% pull(mean_size)
  novel_matrix <- matrix(0, nrow = n_perm, ncol = n_rare)
  ...
  for (p in 1:n_perm) {
    ...
    for (i in 1:n_rare) {
      present <- rownames(pav_mat)[pav_mat[, sub_gids[i]] == 1]
      n_new <- length(setdiff(present, pool))
      pool <- union(pool, present)
      novel_matrix[p, i] <- n_new
    }
  }
  tibble(species = species_name, n_genomes = 1:n_rare,
         mean_novel = colMeans(novel_matrix), sd_novel = apply(novel_matrix, 2, sd),
         mean_size = sp_size)
}

novel_list <- map(sp_levels, calc_novel_rarefied)
novel_df <- bind_rows(novel_list) %>%
  mutate(species = factor(species, levels = sp_levels),
         novel_pct = mean_novel / mean_size * 100,
         sd_pct = sd_novel / mean_size * 100)

novel_smooth <- novel_df %>% group_by(species) %>%
  mutate(smooth_pct = pmax(predict(loess(novel_pct ~ n_genomes, span = 0.3, degree = 1)), 0)) %>%
  ungroup()

novel_end <- novel_smooth %>% group_by(species) %>% slice_max(n_genomes, n = 1) %>% ungroup() %>%
  mutate(label = sprintf("+%.2f%%", smooth_pct),
         x_offset = case_when(
           species == "Borreliella bavariensis" ~ 1.5,
           species == "Borreliella afzelii"     ~ 2.5,
           species == "Borreliella burgdorferi" ~ 3.5,
           species == "Borreliella garinii"     ~ 4.5,
           TRUE ~ 3
         ),
         x_label = n_genomes + x_offset,
         y_label = case_when(
           species == "Borreliella garinii"       ~ 0.50,
           species == "Borreliella afzelii"     ~ 0.32,
           species == "Borreliella burgdorferi" ~ 0.18,
           species == "Borreliella bavariensis" ~ 0.08,
           TRUE ~ smooth_pct
         ))

cat("\n=== Diluted end-point novel gene rate (n=34) ===\n")
print(novel_end %>% select(species, n_genomes, smooth_pct, novel_pct))

legend_labels_b <- map_chr(sp_levels, function(sp) {
  n_orig <- n_per_species[sp]
  sp_short <- gsub("Borreliella ", "B. ", sp)
  if (n_orig == N_RARE) {
    sprintf("italic(\'%s\') ~ \' (n=%d)\'", sp_short, n_orig)
  } else {
    sprintf("italic(\'%s\') ~ \' (n=%d/%d)\'", sp_short, N_RARE, n_orig)
  }
})
...
novel_plot <- ggplot(novel_smooth, aes(x = n_genomes, y = smooth_pct, color = species)) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray40", linewidth = 0.7) +
  annotate("text", x = 28, y = 1.15, label = "Saturation threshold (1%)", 
  ...
  geom_segment(data = novel_end,
               aes(x = n_genomes, xend = x_label - 0.3, 
                   y = smooth_pct, yend = y_label, color = species),
               linetype = "dotted", linewidth = 0.4, alpha = 0.6, show.legend = FALSE) +
  ...
  coord_cartesian(xlim = c(0, 42), ylim = c(0, 5.2)) +
  scale_x_continuous(breaks = seq(0, 34, 5), expand = expansion(mult = c(0.02, 0.05))) +
...
print(novel_plot)
ggsave(file.path(out_dir, "novel_gene_rate_diluted.png"), novel_plot, 
       width = 7, height = 5, dpi = 300, bg = "white")

cat("\n=== Saturation status (threshold = 1%) ===\n")
novel_end %>% mutate(status = ifelse(smooth_pct < 1, "Saturated (<1%)", "Unsaturated (>1%)"),
                     label = sprintf("%s: %.2f%% (%s)", species, smooth_pct, status)) %>%
  pull(label) %>% cat(sep = "\n")