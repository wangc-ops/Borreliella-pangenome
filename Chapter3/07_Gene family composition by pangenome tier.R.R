# ============================================================
# Gene family composition by pangenome tier (species-level)
# Stacked bar: Core / Soft-core / Shell / Cloud proportions
# ============================================================
...
# --- 6. Global tier (based on matched genomes) ---
...
# --- 7. Species-level tier composition (gene family counts) ---
species_tier <- map_dfr(sp_levels, function(sp) {
  gids <- matched_dt[species == sp, raw_col]
  sp_mat <- pav_sub[, c("Gene", gids), with = FALSE]
  present <- rowSums(sp_mat[, -"Gene"]) > 0
...
# --- 8. Plot ---
totals <- species_tier %>%
  distinct(species_short, total) %>%
  mutate(label_top = sprintf("Sigma == %d", total))

p_comp <- ggplot(species_tier, aes(x = species_short, y = prop, fill = tier)) +
  geom_bar(stat = "identity", width = 0.65, color = "black", linewidth = 0.3) +
  geom_text(aes(label = sprintf("%.1f%%", prop)), 
            position = position_stack(vjust = 0.5), 
            size = 3, color = "black") +
  geom_text(data = totals, aes(x = species_short, y = 102, label = label_top),
  ...
  labs(x = "Species", y = "Proportion of gene families (%)") +
...
# Optional save
# ggsave(file.path(out_dir, "gene_family_tier_composition.png"), p_comp,
#        width = 6.5, height = 5.5, dpi = 300, bg = "white")

# Optional supplementary table
# fwrite(species_tier, file.path(out_dir, "gene_family_tier_data.csv"))